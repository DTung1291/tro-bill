...
-- Hồ sơ quyết định hóa đơn điện tử theo từng workspace. Đây là cấu hình pháp
-- lý/tích hợp, tách khỏi bill nội bộ và không chứa API key của nhà cung cấp.
...
-- Danh mục tài khoản nhận tiền của chủ trọ. Mỗi khu có thể chọn một tài khoản
-- riêng hoặc để trống để kế thừa tài khoản mặc định; một tài khoản được phép
-- phục vụ nhiều khu.
...
CREATE TABLE IF NOT EXISTS rental_reservations (
  ...
  CONSTRAINT rental_reservations_dates_valid CHECK (
    expires_on >= reserved_on AND expected_move_in_on >= reserved_on
  ),
...
-- Cổng công khai theo hợp đồng để khách thuê gửi yêu cầu sửa chữa. Token chỉ
-- lưu SHA-256; yêu cầu giữ snapshot phòng/khách để không mất lịch sử khi state
-- thay tenants/rooms. Phần phân công và chuyển trạng thái được mở ở migration sau.
CREATE TABLE IF NOT EXISTS tenant_maintenance_portal_links (
  ...
  CONSTRAINT tenant_maintenance_portal_expiry_valid CHECK (expires_at > created_at),
...
CREATE TABLE IF NOT EXISTS tenant_maintenance_requests (
  ...
  CONSTRAINT tenant_maintenance_requests_time_valid CHECK (updated_at >= created_at)
);
...
-- Bản phát hành ban đầu không bị ghi đè khi khách trả phòng. Bốn trường final
-- tạo snapshot cuối cùng ghi một lần, còn issued_* giữ nguyên để audit.
ALTER TABLE rent_invoices
...
-- Sổ trạng thái HĐĐT do adapter nhà cung cấp ghi. Trình duyệt chỉ đọc; không có
-- nút phát hành khi sandbox/API contract và credential chưa được xác minh.
...
CREATE TABLE IF NOT EXISTS rent_payment_channels (
  ...
  CONSTRAINT rent_payment_channels_bank_account_owner_fk
...
CREATE TABLE IF NOT EXISTS rent_bank_transactions (
  ...
  CONSTRAINT rent_bank_transactions_bank_account_owner_fk
    FOREIGN KEY (user_id, bank_account_id)
    REFERENCES rent_bank_accounts(user_id, id) ON DELETE RESTRICT,
...
BEGIN
  -- Advisory lock giữ đúng một luồng thay đổi số dư/tài khoản mà không buộc
  -- runtime role append-only phải có quyền UPDATE trên bảng account.
...
-- Biên quyết toán cuối cùng chụp toàn bộ đối chiếu hóa đơn/cọc. Bảng chỉ thêm
-- mới; tiền thật vẫn đi qua payment/deposit ledger hiện hữu trong transaction.
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='tro_bill_runtime') THEN
    EXECUTE 'REVOKE UPDATE ON rent_invoices FROM tro_bill_runtime';
    EXECUTE 'GRANT SELECT, INSERT ON rent_invoices TO tro_bill_runtime';
    EXECUTE 'GRANT UPDATE (room_name_snapshot, issued_total_vnd, detail_snapshot, final_total_vnd, final_detail_snapshot, finalization_contract_id, finalized_at, updated_at) ON rent_invoices TO tro_bill_runtime';
    EXECUTE 'REVOKE UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON rental_final_settlements FROM tro_bill_runtime';
    EXECUTE 'GRANT SELECT, INSERT ON rental_final_settlements TO tro_bill_runtime';
    EXECUTE 'GRANT USAGE, SELECT ON SEQUENCE rental_final_settlements_id_seq TO tro_bill_runtime';
  END IF;
END $$;
