...
test('health check công khai xác minh kết nối database và không cache', async (t) => {
  ...
  db.query = async sql => {
    ...
    assert.match(sql, /rental_contract_notifications_owner_fk/);
    assert.match(sql, /rental_contract_notifications_unique/);
    assert.match(sql, /account_memberships/);
    assert.match(sql, /account_memberships_owner_shape_valid/);
    assert.match(sql, /users_assign_owner_account_membership/);
    return { rows: [{
